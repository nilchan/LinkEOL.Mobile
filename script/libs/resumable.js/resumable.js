...
"use strict";
  ...
  var Resumable = function(opts){
    ...
    $.defaults = {
      ...
      permanentErrors:[400, 401, 404, 406, 415, 416, 500, 501],
      maxFiles:undefined,
      ...
      $h.each(fileList, function(file){
        ...
        if(o.fileType.length > 0){
			...
			for(var index in o.fileType){
				var extension = o.fileType[index]; //'.' + o.fileType[index];
      ...
      $.send = function(){
        ...
        $h.each(customQuery, function(k,v){
          query[k] = v;
				...
				//不兼容的不采用断点续传	Modified by Nil. 2015-12-30
        //var func   = ($.fileObj.file.slice ? 'slice' : ($.fileObj.file.mozSlice ? 'mozSlice' : ($.fileObj.file.webkitSlice ? 'webkitSlice' : 'slice'))),
        //bytes = $.fileObj.file[func]($.startByte,$.endByte),
        var func = 'slice',
        bytes = $.fileObj.file.slice ? $.fileObj.file[func]($.startByte,$.endByte) : $.fileObj.file,
        data   = null,
        ...
        if ($.getOpt('method') === 'octet') {
          ...
          // Add data from the query options
          data = new FormData();
          /*$h.each(query, function(k,v){					//Modified by Nil. 2015-12-30
            data.append(parameterNamespace+k,v);
          });*/
          data.append(parameterNamespace+$.getOpt('fileParameterName'), bytes);
					
          var params = [];												//Added by Nil. 2015-12-30
          $h.each(query, function(k,v){
            params.push([encodeURIComponent(parameterNamespace+k), encodeURIComponent(v)].join('='));
          });
          target = $h.getTarget(params);
    ...
    $.assignBrowse = function(domNodes, isDirectory){
      ...
      $h.each(domNodes, function(domNode) {
        ...
        if(domNode.tagName==='INPUT' && domNode.type==='file'){
          ...
          input.setAttribute('accept', 'video/*;capture=camcorder');	//Added by Nil. 20151229
